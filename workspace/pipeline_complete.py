...
# dependency: tdw>=1.13.0  pillow  numpy
...
def main():
    ...
    # Construct agent from central camera
    central = next(cam for cam in meta["cameras"] if cam["id"] == "central")
    agent = {
        "name": "agent",
        "pos": [central["position"]["x"], central["position"]["z"]],
        "ori": yaw_to_ori(central["rotation"]["y"])
    }
    ...
    print(f"Generated RAGEN room JSON at: {room_path}")

    print(f"pipeline pruned complete → {out_dir.resolve()}")
